...
class BookCategoryRepository {
  ...
  Stream<BookCategoryLoading> loadBooksByReadingStatus(BookReadingStatus bookReadingStatus) {
    return _bookService.loadBooksByReadingStatus(bookReadingStatus).map((VolumeCategoryLoading volumeCategoryLoading) {
      final books = volumeCategoryLoading.volumes.map((Volume volume) =>
          Book(
              title: volume.volumeInfo.title,
              industryIds: volume.volumeInfo.industryIdentifiers!
                  .map((VolumeIndustryIdentifier volumeIndustryIdentifier) =>
              ...
              coverImageLarge: volume.volumeInfo.imageLinks?.small)).toList();

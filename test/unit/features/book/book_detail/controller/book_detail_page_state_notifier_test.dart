...
void main() {
  ...
  late BookReadingDetail bookReadingDetail;
  ...
  setUp(() {
    ...
    bookReadingDetail = BookReadingDetail(book: Book(title: 'a', industryIds: industryIds), status: bookReadingDetail.status);
  });

  void arrangeRepositoryReturnsWithBook() {
    when(() => bookDetailRepository.loadBookByIndustryIds(industryIds)).thenAnswer((invocation) => Future.value(bookReadingDetail));
  }
  ...
  group('BookDetailPageStateNotifier', () {
    group('loadBookByExternalId', () {
      test('should emit inProgress and success states if a book was given from the repositrx', () async {
        ...
        expect(bookDetailPageStateNotifier.state, BookDetailPageState.loadingSuccess(bookReadingDetail));
      });
    });
  });
}

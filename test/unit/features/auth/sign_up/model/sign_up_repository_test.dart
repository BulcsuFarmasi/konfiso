...
void main() {
  group('SignUpRepository', () {
    ...
    void arrangeServiceThrowsException(String message) {
      when(() => authService.signUp(email, password)).thenThrow(NetworkException(message));
    }
    ...
    group('signUp', () {
      ...
      test('should throw email exists exception if email exists', () {
        ...
        expect(signUpRepository.signUp(email, password),
            throwsA(predicate((e) => e is SignUpException && e.error == SignUpError.emailExists)));
      });
      test('should throw email operation not allowed if operation is not allowed', () {
        ...
        expect(signUpRepository.signUp(email, password),
            throwsA(predicate((e) => e is SignUpException && e.error == SignUpError.operationNotAllowed)));
      });
      test('should throw too many attempts if too many attempts were made', () {
        ...
        expect(signUpRepository.signUp(email, password),
            throwsA(predicate((e) => e is SignUpException && e.error == SignUpError.tooManyAttempts)));
      });
      test('should throw other if error is unknown', () {
        arrangeServiceThrowsException('b');

        expect(signUpRepository.signUp(email, password),
            throwsA(predicate((e) => e is SignUpException && e.error == SignUpError.other)));
      });
    });
  });
}
